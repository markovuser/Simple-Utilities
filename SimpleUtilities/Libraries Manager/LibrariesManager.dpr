...
begin
  HM := OpenMutex(MUTEX_ALL_ACCESS, false,
    'Simple Utilities Libraries Manager');
  Result := (HM <> 0);
  if HM = 0 then
    HM := CreateMutex(nil, false, 'Simple Utilities Libraries Manager');
end;
...
begin
  ...
  Application.Title := 'Libraries Manager';
