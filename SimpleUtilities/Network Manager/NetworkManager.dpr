...
begin
  HM := OpenMutex(MUTEX_ALL_ACCESS, false, 'Simple Utilities Network Manager');
  Result := (HM <> 0);
  if HM = 0 then
    HM := CreateMutex(nil, false, 'Simple Utilities Network Manager');
end;
...
begin
  ...
  Application.Title := 'Network Manager';
