...
begin
  HM := OpenMutex(MUTEX_ALL_ACCESS, false,
    'Simple Utilities File EnCrypt DeCrypt');
  Result := (HM <> 0);
  if HM = 0 then
    HM := CreateMutex(nil, false, 'Simple Utilities File EnCrypt DeCrypt');
end;
...
begin
  ...
  Application.Title := 'File EnCrypt DeCrypt';
