...
begin
  HM := OpenMutex(MUTEX_ALL_ACCESS, false, 'Simple Utilities Registry Search');
  Result := (HM <> 0);
  if HM = 0 then
    HM := CreateMutex(nil, false, 'Simple Utilities Registry Search');
end;
...
begin
  ...
  Application.Title := 'Registry Search';
